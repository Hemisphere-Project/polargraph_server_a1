...
void curves_drawCurve(long x, long y, long fx, long fy, long cx, long cy) {
  curves_drawCurve2(x,y,fx,fy,cx,cy); 
}

void curves_drawCurve2(long x, long y, long fx, long fy, long cx, long cy) {
  // Draw a Quadratic Bezier curve from (x, y) to (fx, fy) using control pt
  // (cx, cy)
  float xt=0;
  float yt=0;

  for (float t=0; t<=1; t+=1) {//t+=.01
    xt = pow((1-t),2) *x + 2*t*(1-t)*cx+ pow(t,2)*fx;
    yt = pow((1-t),2) *y + 2*t*(1-t)*cy+ pow(t,2)*fy;
    
    float pa = getMachineA(x, y);
    float pb = getMachineB(x, y);
    
    float pta = getMachineA(xt, yt);
    float ptb = getMachineB(xt, yt);
    
    float deltaA = pta - pa;    // distance each must move (signed)
    float deltaB = ptb - pb;
...
void curves_drawCurve(long x, long y, long fx, long fy, long cx, long cy, int speedOfSegment) {
  ...
  for (float t=0; t<=1; t+=.01) {//t+=.01
    ...
    float pA = getMachineA(xt, yt);
    float pB = getMachineB(xt, yt);
    changeLength(pA, pB);
    //changeLength(xt, yt);
    reportingPosition = true;
    usingAcceleration = true;
